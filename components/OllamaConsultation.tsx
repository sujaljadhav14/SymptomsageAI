/**
 * OllamaConsultation.tsx — streaming TTS + 3D robot character (clean version)
 */
...
function extractSentences(text: string): { sentences: string[]; remainder: string } {
  ...
  while ((match = sentenceEnd.exec(text)) !== null) {
    const s = text.slice(lastIndex, match.index + match[0].trimEnd().length).trim();
...
class TTSQueue {
  ...
  private processNext() {
    ...
    utt.onstart = () => { this.onStateChange('speaking'); this.startLipSync(); };
  ...
  private startLipSync() {
    ...
    this.lipSyncInterval = setInterval(() => {
      const v = 0.25 + 0.35 * Math.abs(Math.sin(t * 4.1)) + 0.20 * Math.abs(Math.sin(t * 9.3)) + 0.15 * Math.abs(Math.sin(t * 2.7));
  ...
  cancel() {
    this.cancelled = true; this.queue = []; this.synth.cancel();
    this.stopLipSync(); this.processing = false; this.onStateChange('idle');
  }
...
const OllamaConsultation: React.FC<OllamaConsultationProps> = ({ onEndSession }) => {
  ...
  const sendToOllama = useCallback(async (userText: string) => {
    ...
    setMessages(prev => [...prev, { id: Math.random().toString(36).slice(2), role: 'user', text: userText, timestamp: new Date() }]);

    try {
      abortRef.current = new AbortController();
      await chatWithOllama(
        [{ role: 'system', content: SYSTEM_INSTRUCTION }, ...conversationRef.current],
        (chunk) => {
      ...
      setMessages(prev => [...prev, { id: Math.random().toString(36).slice(2), role: 'assistant', text: finalText, timestamp: new Date() }]);
      setStreamingText('');
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onstart = () => { setIsListening(true); setSessionState('listening'); setCurrentTranscript(''); };
    ...
    recognition.onend = () => {
      ...
      if (t) sendToOllama(t); else setSessionState('ready');
    };
    recognition.onerror = (e: any) => {
      setIsListening(false);
      if (e.error !== 'no-speech') { setError(`Mic error: ${e.error}`); setSessionState('error'); }
  ...
  const skipSpeaking = () => { ttsQueueRef.current.cancel(); setSessionState('ready'); };
  const endSession = () => { ttsQueueRef.current.cancel(); recognitionRef.current?.stop(); abortRef.current?.abort(); onEndSession(messages); };

  useEffect(() => { return () => { ttsQueueRef.current.cancel(); abortRef.current?.abort(); }; }, []);

  // ── RENDER ─────────────────────────────────────────────────────────────────
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 rounded-2xl overflow-hidden">

      {/* ── HEADER ─────────────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.06] bg-black/30 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          <div className={`w-2 h-2 rounded-full ${ollamaStatus?.available ? 'bg-green-400 animate-pulse' : 'bg-red-400'}`} />
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            {(['idle', 'listening', 'thinking', 'speaking'] as CharacterState[]).map(s => (
              <div key={s} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${characterState === s ? 'scale-150 opacity-100' : 'opacity-20'}`}
                style={{ backgroundColor: s === 'listening' ? '#60a5fa' : s === 'thinking' ? '#a78bfa' : s === 'speaking' ? '#34d399' : '#94a3b8' }} />
            ))}
          </div>
          {ollamaStatus?.available && (
            <button onClick={endSession} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 text-red-400 rounded-lg text-xs font-semibold hover:bg-red-500/20 border border-red-500/20 transition-all">
      ...
      {/* ── 3D ROBOT — full width, fixed 300px stage ───────────────────────── */}
      <div className="relative w-full flex-shrink-0 border-b border-white/[0.06]" style={{ height: 300 }}>
        <div className="absolute inset-0 bg-gradient-to-b from-indigo-950/40 via-transparent to-transparent pointer-events-none z-10" />
        <RobotCharacter3D state={characterState} volume={lipVolume} />
      </div>

      {/* ── STATUS + CHAT + CONTROLS ────────────────────────────────────────── */}
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <AnimatePresence mode="wait">
          {sessionState === 'checking' && (
            <motion.div key="checking" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-500/10 border-b border-blue-500/20 text-blue-300 text-xs flex-shrink-0">
          ...
          {sessionState === 'error' && error && (
            <motion.div key="error" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className="px-5 py-3 bg-red-500/10 border-b border-red-500/20 text-xs flex-shrink-0">
              <div className="flex items-start gap-2 text-red-300">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-red-400/80">
                    ...
                    {' '}then <code className="bg-red-900/30 px-1.5 py-0.5 rounded">ollama pull llama3</code>
                  </p>
                  <button onClick={checkOllama} className="mt-1 px-2 py-1 bg-red-500/20 hover:bg-red-500/30 rounded text-red-300 font-bold">Retry</button>
                </div>
              </div>
            </motion.div>
          )}
          {sessionState === 'ready' && (
            <motion.div key="ready" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className="flex items-center gap-2 px-5 py-2 bg-emerald-500/10 border-b border-emerald-500/20 text-emerald-300 text-xs flex-shrink-0">
              <CheckCircle2 className="w-3.5 h-3.5" /> Ready — tap the mic and speak
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 min-h-0">
              ...
              <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className={`max-w-[88%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'bg-white/[0.07] text-slate-200 rounded-tl-sm border border-white/[0.08]'
                }`}>{msg.text}</div>
                <span className="text-[10px] text-slate-600 mt-1 px-1">
                  {msg.role === 'user' ? 'You' : 'Sage AI'} · {msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </motion.div>
            ))
          )}
          ...
          {streamingText && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-start">
              <div className="max-w-[88%] rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm leading-relaxed bg-violet-500/10 text-slate-200 border border-violet-500/20">
                {streamingText}
                <span className="inline-block w-0.5 h-3.5 ml-1 bg-violet-400 rounded-full align-middle animate-pulse" />
              </div>
          ...
          {currentTranscript && isListening && (
            <div className="flex flex-col items-end">
              <div className="max-w-[88%] bg-slate-800/80 text-slate-300 rounded-2xl rounded-tr-sm px-4 py-2.5 text-sm opacity-70">
                {currentTranscript}<span className="text-slate-500">…</span>
              </div>
            </div>
          )}
        ...
        {/* Controls */}
        <div className="p-4 border-t border-white/[0.06] bg-black/10 flex items-center justify-center gap-4 flex-shrink-0">
          {sessionState === 'ready' && (
            <div className="flex flex-col items-center gap-2">
              <motion.button onClick={startListening} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.93 }}
          ...
          {sessionState === 'speaking' && (
            <div className="flex flex-col items-center gap-2">
              ...
              <button onClick={skipSpeaking} className="text-[10px] text-slate-500 hover:text-slate-300 underline transition-colors">Skip →</button>
            </div>
          )}
          {sessionState === 'error' && (
            <button onClick={checkOllama} className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold transition-all border border-slate-700">
              Retry Connection
            </button>
          )}
        </div>
